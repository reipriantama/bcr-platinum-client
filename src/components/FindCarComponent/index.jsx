...
// e punya property target yang merupakan inputan, value adalah hasil dari inputan dari input form kita, untuk yang option kita letak di select agar bisa dibaca valuenya
    ...
    const [name, setName] = useState(JSON.parse(sessionStorage.getItem("selectedCar")).name);
    const [category, setCategory] = useState(JSON.parse(sessionStorage.getItem("selectedCar")).category);
    const [price, setPrice] = useState(JSON.parse(sessionStorage.getItem("selectedCar")).price);
    const [minPrice, setMinPrice] = useState(JSON.parse(sessionStorage.getItem("selectedCar")).minPrice);
    const [maxPrice, setMaxPrice] = useState(JSON.parse(sessionStorage.getItem("selectedCar")).maxPrice);
    const [status, setStatus] = useState(JSON.parse(sessionStorage.getItem("selectedCar")).status);
    const [data, setData] = useState([]);

    const [selectedCar, setSelectedCar] = useState(JSON.parse(sessionStorage.getItem("selectedCar")));
    ...
    const handleSetPrice = (e) => {
        if(e.target.value === "default") {
            ...
            setMinPrice(0);
            setMaxPrice(1000000);
            setSelectedCar({...selectedCar, price: "", minPrice: 0, maxPrice: 1000000});
        }
            ...
            setMinPrice(600000);
            setMaxPrice(800000);
            setSelectedCar({...selectedCar, price: "large", minPrice: 600000, maxPrice: 800000});
        }
    };
  ...
  return (
    ...
    <form onSubmit={(e) => onSubmit(e)}>
        <div className="row">
            ...
            <input type="string" className="form-control" id="inputEmail4" placeholder="Ketik nama/tipe mobil" value={name} onChange={(e) => { handleSetName(e); }} />
            </div>
            <div className="col">
                <label for="inputCapacity">Kategori</label>
                    <select id="inputCapacity" className="form-select" value={category} onChange={(e) => { handleSetCategory(e); }}>
            ...
            <div className="col">
                ...
                <select id="inputPrice" className="form-select" value={price} onChange={(e) => { handleSetPrice(e); }}>
            ...
            <div className="col">
                <div className="row">
                    <div className="col">
                        ...
                        <select id="inputState" className="form-select" value={status} onChange={(e) => { handleSetStatus(e); }}>
